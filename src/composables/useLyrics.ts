...
export const useLyrics = () => {
  ...
  interface LyricMerged { time: number; ori: string; tran?: string; roma?: string }
  ...
  const activeSingleLyrics = computed<LyricLine[]>(() => {
    return mergedLines.value.map(m => ({
      time: m.time,
      text: [m.ori, state.showTrans && m.tran ? m.tran : undefined, state.showRoma && m.roma ? m.roma : undefined]
        .filter(Boolean)
        .join('\n'),
    }))
  })
  ...
  const fetchLyrics = async (id?: string | number, force = false) => {
    try {
      if (!id) { state.lyricsOriginal = []; state.lyricsTrans = []; state.lyricsRoma = []; lastId.value = null; return }

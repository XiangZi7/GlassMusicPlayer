...
const load = async () => {
  ...
  try {
    ...
    state.playlists = playlists.map((pl: any) => ({ id: pl?.id || 0, name: pl?.name || '歌单', coverImgUrl: pl?.coverImgUrl || '', trackCount: pl?.trackCount || 0 }))
...
<template>
  <div class="flex-1 overflow-auto px-3 pb-6">
    ...
    <template v-else>
      <section class="mb-6">
        <h2 class="mb-2 text-sm font-semibold text-primary">热门歌单</h2>
      ...
      <section>
        <h2 class="mb-2 text-sm font-semibold text-primary">热门歌曲</h2>

...
const load = async (id: string) => {
  try {
    ...
    const song = (detailRes as any)?.songs?.[0] || (detailRes as any)?.data?.songs?.[0] || (detailRes as any)?.data?.[0]
    if (song) {
      state.name = song?.name || ''
      state.artist = Array.isArray(song?.ar) ? song.ar.map((a: any) => a.name).join(' / ') : Array.isArray(song?.artists) ? song.artists.map((a: any) => a.name).join(' / ') : ''
...
const toggleLike = () => { state.liked = !state.liked }
</script>
      ...
      <section class="mb-4">
        <div class="flex items-center gap-3">
          <div class="h-16 w-16 shrink-0 overflow-hidden rounded-xl">
            <img v-if="state.cover" :src="state.cover + '?param=300y300'" alt="cover" class="h-full w-full object-cover" />
